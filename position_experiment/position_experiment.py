...
def main():
    y0 = [0, 1]  # Start at 0, driving with 1 m/s
    step_size = 10.0  # [s]
    t_start = 0
    t_end = 2000
    ...
    y_true = [y0[0] + t * y0[1] for t in t_eval]
    ...
    for epoch in tqdm(range(N_epochs), desc="vanilla: training epoch"):
        ...
        y_train_v = y_train.view(1, len(y_train))
        loss_collocation = torch.nn.functional.mse_loss(out, y_train_v)
    ...
    plot_prediction(t_eval, y_pred[0], y_true, "x(t)", ax1, "b")
    # plot_prediction(y_pred[1], y_true[1], "ω(t)", ax2, "r")

    t_samples = t_train.detach().cpu()
    x_samples = y_true[::subsample_every]
